    ...
    public class SRanipal_EyeFocusSample_LEM : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            foreach (GazeIndex index in GazePriority)
            {
                ...
                if (eye_focus) //change stuff here
                {
                    ArrayObjects arrayObject = FocusInfo.transform.GetComponent<ArrayObjects>();
                    //DartBoard dartBoard = FocusInfo.transform.GetComponent<DartBoard>();
                    if (arrayObject != null) arrayObject.Focus();
                    //if (dartBoard != null) dartBoard.Focus(FocusInfo.point);                    
                    break;
                }
                else
                {
                    ArrayObjects[] arrayObjects = transform.GetComponentsInChildren<ArrayObjects>();
                    foreach (ArrayObjects ao in arrayObjects)
                    {
                        ao.Unfocus();
                    }                    
                    break;
